...
const slideDown = (target, duration=ANIMATION_DURATION) => {
    ...
    if (html.getAttribute("dir") == "rtl") {
        if (leftEdge < 0) {
            ...
            if (listItem.closest('ul').classList.contains('force-left') && rightEdge < document.documentElement.clientWidth) {
                target.classList.add('force-left');
            } else {
...
const slideToggle = (target, duration=ANIMATION_DURATION) => {
    ...
    if (!((html.getAttribute("data-nav-style") === "menu-hover" && html.getAttribute("toggled") === "menu-hover-closed" && window.innerWidth >= 992) || (html.getAttribute("data-nav-style") === "icon-hover" && html.getAttribute("toggled") === "icon-hover-closed" && window.innerWidth >= 992)) && target && target.nodeType != 3) {
...
firstLevelItems.forEach( (element) => {
    element.addEventListener("click", () => {
        let html = document.querySelector("html");
        if ((html.getAttribute("data-nav-style") != "menu-hover" && html.getAttribute("data-nav-style") != "icon-hover") || window.innerWidth < 992 || (!html.getAttribute("toggled") && html.getAttribute("data-nav-layout") != "horizontal")) {
...
( () => {
    ...
    if (!localStorage.getItem("Syntolayout") && !localStorage.getItem("Syntonavstyles") && !localStorage.getItem("Syntoverticalstyles") && !document.querySelector(".landing-body") && document.querySelector("html").getAttribute("data-nav-layout") !== "horizontal") {
                ...
                if (html.getAttribute("toggled") === "double-menu-open") {
                    ...
                    if (sidemenu) {
                        ...
                        if (sidemenu.nextElementSibling) {
                            ...
                            document.querySelector("html").setAttribute("toggled", "double-menu-close");
                        }
                    }
                }
    ...
    if (document.querySelector("html").getAttribute("data-nav-layout") === "horizontal" && window.innerWidth > 992) {
        ...
        if (leftForceItem) {
            if (document.querySelector("html").getAttribute("dir") != "rtl") {
                ...
                console.log(leftForceItem, check);
                if (check < document.documentElement.clientWidth) {
                    leftForceItem.classList.remove("force-left");
                } else if (leftForceItem.getBoundingClientRect().left < 0) {
                    if ((document.documentElement.getAttribute("data-nav-style") == "menu-hover" || document.documentElement.getAttribute("data-nav-style") == "icon-hover") || document.documentElement.clientWidth > 992) {
        ...
        elements.forEach( (e) => {
            if (isElementVisible(e)) {
                ...
                if (document.documentElement.getAttribute("dir") == "rtl") {
                    ...
                    if (ele.right > document.documentElement.clientWidth) {
                        if (e.classList.contains("child1")) {
                            e.classList.remove("force-left");
                        } else {
                            e.classList.add("force-left");
...
function HoverToggleInnerMenuFn(event) {
    ...
    if (element && html.getAttribute("data-nav-layout") == "horizontal" && (html.getAttribute("data-nav-style") == "menu-hover" || html.getAttribute("data-nav-style") == "icon-hover")) {
        ...
        if (html.getAttribute("dir") == "rtl") {
            ...
            console.log(rightEdge, document.documentElement.clientWidth);
            // Check if moving out to the right
            if (rightEdge > document.documentElement.clientWidth) {
                target.classList.add('force-left');
            } else {
...
document.querySelectorAll(".side-menu__item").forEach( (element) => {
    element.addEventListener("click", () => {
        ...
        if (ulMenu) {
            ...
            const isOverflowing = elementRect.right > window.innerWidth || elementRect.bottom > window.innerHeight;
            if (isOverflowing && document.querySelector("html").getAttribute("data-nav-layout") == "horizontal" && document.querySelector("html").getAttribute("dir") == "ltr") {
            ...
            if (isOverflowingRTL && ulMenu.scrollWidth != 0 && document.querySelector("html").getAttribute("data-nav-layout") == "horizontal" && document.querySelector("html").getAttribute("dir") == "rtl") {
...
document.querySelector(".main-content").addEventListener("click", () => {
    document.querySelectorAll(".slide-menu").forEach( (ele) => {
        if (document.querySelector("html").getAttribute("toggled") == "menu-click-closed" || document.querySelector("html").getAttribute("toggled") == "icon-click-closed") {
